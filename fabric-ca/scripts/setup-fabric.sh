...
function generateChannelArtifacts() {
  ...
  local CHANNEL_TX_FILE1=/$DATA/${CHANNEL_NAME}1.tx
  log "Generating channel configuration transaction at ${CHANNEL_TX_FILE1}"
  configtxgen -profile OrgsChannelOne -outputCreateChannelTx ${CHANNEL_TX_FILE1} -channelID ${CHANNEL_NAME}1
  if [ "$?" -ne 0 ]; then
    fatal "Failed to generate channel configuration transaction"
  fi

  local CHANNEL_TX_FILE2=/$DATA/${CHANNEL_NAME}2.tx
  log "Generating channel configuration transaction at ${CHANNEL_TX_FILE2}"
  configtxgen -profile OrgsChannelTwo -outputCreateChannelTx ${CHANNEL_TX_FILE2} -channelID ${CHANNEL_NAME}2
  if [ "$?" -ne 0 ]; then
    fatal "Failed to generate channel configuration transaction"
  fi
  ...
  for ORG in $PEER_ORGS_CH1; do
     local ANCHOR_TX_FILE_TMP=/${DATA}/orgs/${ORG}/anchors-${CHANNEL_NAME}1.tx
     initOrgVars $ORG
     log "Generating anchor peer update transaction for $ORG at $ANCHOR_TX_FILE_TMP"
     configtxgen -profile OrgsChannelOne -outputAnchorPeersUpdate $ANCHOR_TX_FILE_TMP -channelID ${CHANNEL_NAME}1 -asOrg $ORG
     if [ "$?" -ne 0 ]; then
  ...
  for ORG in $PEER_ORGS_CH2; do
     local ANCHOR_TX_FILE_TMP=/${DATA}/orgs/${ORG}/anchors-${CHANNEL_NAME}2.tx
     initOrgVars $ORG
     log "Generating anchor peer update transaction for $ORG at $ANCHOR_TX_FILE_TMP"
     configtxgen -profile OrgsChannelTwo -outputAnchorPeersUpdate $ANCHOR_TX_FILE_TMP -channelID ${CHANNEL_NAME}2 -asOrg $ORG
     if [ "$?" -ne 0 ]; then
        fatal "Failed to generate anchor peer update for $ORG"
     fi
  done
